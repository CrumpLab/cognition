---
title: 'PSYC 2530: Introduction to Cognitive Psychology'
author: "Matthew J. C. Crump"
date: "Last compiled `r format(Sys.time(), '%m/%d/%y')`"
subtitle: Fall 2023, Welcome!
output:
  revealjs::revealjs_presentation:
    includes:
      in_header: header.html
    self_contained: false
    theme: white
    transition: fade
    center: false
    incremental: true
    css: style.css
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
library(fontawesome)
```

#  Roadmap

::: {.pop_box_2} 
1 What is this course about?
:::
::: {.grey_box} 
2 Course Resources
:::
::: {.grey_box} 
3 Syllabus and Logistics
:::
::: {.grey_box} 
4 Questions
:::


# What is Cognition?

::: {.bigger}
1. Questions
2. Methods
3. Findings
4. Explanations
5. Applications
6. Implications
:::

#  Roadmap

::: {.grey_box} 
1 What is this course about?
:::
::: {.pop_box_2} 
2 Course Resources
:::
::: {.grey_box} 
3 Syllabus and Logistics
:::
::: {.grey_box} 
4 Questions
:::


# Course Resources

::: {.bigger}
1. Blackboard
2. Course Website
3. Textbook
:::

# Blackboard

We will use blackboard for:

1. Course Announcements
    - Make sure your email is up to date
2. Weekly learning modules, quizzes, assignments, and assessments
3. Links to material on course website
4. Asking questions (use discussion forum)

# Course Website

We will also use an external course website:

<https://www.crumplab.com/cognition/>

1. Most current Syllabus
2. Links to textbook
3. Weekly learning modules
4. Slide decks
4. Other resources as needed

# Course Textbook

<https://www.crumplab.com/cognition/textbook/>

:::: {.row}
::: {.col-md-6}

The textbook is:

1. A free open-educational resource
2. Web-book, available as pdf and epub
3. Still in progress.

:::

::: {.col-md-6}

```{r, out.width="60%"}
knitr::include_graphics("imgs/cover.png")
```


:::
::::

# Other resources

Throughout the semester additional reading material may be assigned. 

Any additional material will be made available for free through blackboard or the course website

#  Roadmap

::: {.grey_box} 
1 What is this course about?
:::
::: {.grey_box} 
2 Course Resources
:::
::: {.pop_box_2} 
3 Syllabus and Logistics
:::
::: {.grey_box} 
4 Questions
:::

# Course Syllabus

[Please read the syllabus](https://www.crumplab.com/cognition/articles/course_docs/Fall_2023_Psyc_2530_syllabus.html).

The most current version of the syllabus is located on the course website.


# Basic overview

15 weeks in the semester

13 Learning module weeks

2 assessments (including final)

[Schedule is posted on the syllabus](https://www.crumplab.com/cognition/articles/course_docs/Fall_2023_Psyc_2530_syllabus.html#course-schedule)

# Assessments and grading

There are 13 learning modules, and three assessments. The grading rubric is below:

| Assignment |  Points| Total|
|:----------|--------:|-------:|
| Weekly Coursework | 2.5-5+ | up to 85|
| Assessment 1 | 5 | 5 |
| Assessment 2 | 5 | 5 |
| Final Assessment | 5  | 5|
|||100|

# Weekly learning modules

1. Posted weekly (or earlier) on Blackboard and Course Website
2. Include readings, quizzes/assignments, supplementary lectures
3. Complete the work within the week (submit by due date)
4. Late work may not be graded
5. Lots of assignment choice (see next slide)

# Assignment choice

- modules have multiple possible assignments, (e.g., quiz and/or writing assignment).

- you choose which assignments to complete.

- Complete as many as you want, max 85 points toward final grade

- If you miss an assignment that is OK, you can do an extra one in a future module

# Assessments

- There are three major assessments worth 5 points each.

- Assessments are self-reflective and used to collaboratively set and achieve goals for engaging with the course material.

- These are low pressure, and more info about them will be released on Blackboard.

# Grading/Ungrading

- Except for the optional weekly quizzes, all assignments and major assessments are self-assessed. 
- Students submit their assignment and recommend their own grades/points for the assignment

- Upon submission of the final assessment, students give a recommendation for their final course grade

- The instructor makes final grading decisions and considers the self-assessments.

# Academic Integrity

It is your responsibility to understand [the CUNY academic integrity policy](https://www.cuny.edu/about/administration/offices/legal-affairs/policies-resources/academic-integrity-policy/) and conduct your behavior according to a high standard of academic integrity.

#  Roadmap

::: {.grey_box} 
1 What is this course about?
:::
::: {.grey_box} 
2 Course Resources
:::
::: {.grey_box} 
3 Syllabus and Logistics
:::
::: {.pop_box_2} 
4 Questions
:::

# Questions

Use the discussion board on Blackboard to ask general questions about the course

# Email

If you are running into issues, email me:

1. Put PSYC 2530 in the subject line
2. Email me at mcrump@brooklyn.cuny.edu 

# Next Steps

Finish the getting started module by taking the quiz on blackboard and completing the introductory post.

Then, you can begin the first official learning module:

L1: What is cognition?




