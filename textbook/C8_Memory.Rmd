...
We're about to finish up this chapter by discussing research on short and long-term memory, as well as working memory. Before we do that, now is a good time to discuss differences between memory tasks, abilities, and processes. When I read the memory literature I am often confused about whether researchers are discussing tasks, abilities, or processes, or some combinations of the three. So, I will define these three aspects common to all cognitive research before we continue. Let's use the immediate memory span as an example. 

### Tasks

The immediate memory span task is most obviously a task that people perform. Most people don't perform this task in everyday life, or most psychological tasks. The immediate memory span task was created as a research instrument. As a research instrument, the task has some controlled properties, and people asked to perform it in a particular way. That is, follow the instructions. For example, if I asked you to remember the following 10 digits (3,5,4,2,3,2,6,5,7,6), and you told me about your favorite movie, then it wouldn't count as "doing the task". Tasks are also made to provide convenient measurements. For example the immediate memory span task yields a number, called the immediate memory span. It is the number of items correctly recalled in the task. 

### Abilities

We can distinguish between the immediate memory span task, and the ability to perform the task. The immediate memory span task is an example of a behavioral task. People perform the task, and some aspect of their behavior in the task is measured. For example, the immediate memory span is a measure of performance in the task. Behavioral measures of performance can reflect the ability to perform a task, but they may not. For example, I have the ability to perform an immediate memory task, but this ability will not be measured if I choose not to perform the task. While I am doing the task, I can also choose to do it in different ways. In other words, I could use multiple abilities to perform the task. I might not even know what abilities I was using. At the very least, when a task is performed, I will use ability to refer to the fact that the task was performed in some way. 

### Processes

In cognitive research tasks and abilities are measured to better understand the underlying cognitive processes involved. We have just described the immediate memory span as task that can be completed. If you follow the instructions and complete the task as intended, then your performance will be measured, potentially as a reflection of your ability. For example, you may have a measured immediate memory span of 5, 7, or 10, or some other number. What does this number mean in relation to a cognitive process? For example, do you have an immediate memory system? If so, does it have a span? My answer is no, not at all. 

Cognitive processes refer to the mechanisms underlying the abilities to perform a task. Tasks produce measures of abilities, but not direct measures of the processes enabling the abilities. Instead, cognitive processes are largely theoretical entities. They are used to explain the measures of behavior. In this way, it is possible for multiple theories to explain the same measurements of task performance. 

I'm bringing these issues to the foreground because it will be useful to distinguish between tasks, abilities, and process explanations when we discuss short and long term memory. It is possible to make short and long term memory tasks. People have the ability to remember some information over very long time periods, and at the same time do not have the ability to remember other information over very short periods. Do these facts about tasks and abilities require different process explanations? That is a research question in cognition that continues to be debated.

## Short and long term memory

I'm guessing you have heard of short-term memory and long-term memory. It's OK if you haven't. These are terms from cognitive psychology that are commonly used to describe memory abilities. We've all forgotten something that just happened, and maybe blamed short-term memory. We've all remembered something from ages ago...thanks to long-term memory. Clearly, memory can last a long time sometimes, and not last a long time other times. What is going on here? Why is memory behaving so differently?
...
Before we look at this idea more closely let me first make a comment about independent systems. First, the distinction between a short-term and long-term memory is between supposedly separate systems. There are lots of examples of separate systems in humans, especially at a physiological level. For example, we could consider different organs in the body as separate independent systems, each doing their own job in the connected network of your body. In terms of brain function, there are also different perceptual systems and brain areas mostly dedicated to processing information from each modality. I think partly because systems like these do exist, that some researchers adopt a "systems-focused" approach to cognition. In this approach, one of the goals is to find all of the systems, and then interrogate them to understand how they work.

Several memory models distinguish between short and long-term memory systems. The basic short-term/long-term distinction marks a branching point in memory research. One side of the branch continued to make distinctions between different kinds of memory systems. Another side of the branch tended to focus more on the algorithms of memory processes. The next two chapters expand on the algorithmic branch. The last part of this chapter describes the multiple-systems approach, starting with the multi-store model.

### Multi-store model

A classic memory model that distinguishes between short and long-term memory is the multi-store model of Atkinson and Shriffrin from 1968. This model is fairly easy to explain and not too complicated, which is one reason why it had a big impact. At the time, it also successfully modeled a number of findings from the memory literature. We will talk about this model in general terms. However, Atkinson and Shiffrin also formalized the operation of their model with mathematical assumptions, which allowed them to more precisely generate predictions from the model. Although the model follows an information processing approach, the underlying metaphor for cognition is more in the vein of the assembly line metaphor from last chapter.

The multi-store model has three major storage locations, a sensory register, a short-term store, and a long-term store. Each storage device is further split by modality. There is a visual sensory register, and an auditory register etc. The short-term store also has different locations for different kinds of information, like auditory information, or visual information. Similarly, the long-term store also has separate locations for different kinds of representations. 

More important, the model describes the operation of control processes allowing information to flow between the different storage locations. An incoming stimulus is first registered in sensory storage. Here there was some probability that the new information could be lost due to forgetting. The sensory stimulus could be sent to the short term store, or it could be sent directly to the long-term store (with some probability). The short and long-term stores had their own control processes and rules for operation.

The short-term store had limited storage locations. It was imagined as buffer between the sensory register and long-term memory. The buffer could keep a representation "alive" for a short while. They used the example of a rehearsal loop. For example, if you are remembering a phone number, you can say it over and over in your head. This is called rehearsal, and it can help you "maintain" the phone number for a longer period of time. Representations in the short-term store could also be lost with some probability.

The long-term store received representations from the sensory register or from short-term memory. This storage location was assumed to have a more robust storage system. It could store many representations for longer periods of time. The long-term store was still vulnerable to information loss. A memory representation could lose its strength over time.

Finally, Atkinson and Shiffrin discussed rules for encoding and retrieval of memory traces. These rules determined how information was encoded into short or long-term memory, and how memory traces were retrieved from those locations.

All together, the model had several parameters that could be used to describe performance in memory tasks. For example, failure to remember something could be explained in the model in several ways. The trace could be lost immediately from the sensory register, it could have made it to short term-memory, but never transferred to long-term memory. These would be considered encoding failures. The trace could have been succesfully transferred to long-term memory, but decayed over time and lost its strength. This would be like Ebbinghaus' "obliviscence", where a memory trace is assumed to disintegrate into nothing. Finally, the model had a role for retrieval to cause problems with remembering. For example, in the model it was possible for a strong memory trace to suffer interference during retrieval. Other memory traces were lying around too, and they would compete for retrieval. This could lead to incorrect remembering, which could appear like forgetting.


Short and long-term memory systems
marks a branching point in memory research. 

When we consider the topic of memory, the concept of a system becomes a little bit more fuzzy. 
